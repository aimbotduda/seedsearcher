/*
 * groupfinder.c - Memory-efficient structure group finder
 * 
 * Finds groups of 3 or 4 structures within a specified radius.
 * Optimized to handle 800M+ structures in under 24GB RAM.
 *
 * Memory-efficient design:
 *   - Minimal 8-byte structure (x, z only)
 *   - Cell coordinates computed on-the-fly during sort
 *   - Larger cell size (4× radius) to reduce cell count
 *   - Compact cell index with contiguous storage
 */
...
#define CELL_SIZE_MULTIPLIER 4  /* cell_size = radius * this (larger = fewer cells, more per cell) */

/* ============================================================================
 * Data Structures - Minimal memory footprint
 * ========================================================================== */

/* Structure position - minimal 8 bytes */
typedef struct {
    int32_t x;
    int32_t z;
} Structure;
...
static Structure *g_structures = NULL;
...
static int64_t g_cell_size = 0;  /* For sort comparison */
...
static inline int64_t dist_sq(const Structure *a, const Structure *b)
{
...
static bool preallocate_structures(size_t file_size)
{
    ...
    uint64_t initial_cap = (estimated_count * 11) / 10;
    ...
    g_structures = malloc(cap * sizeof(Structure));
    if (!g_structures) {
        fprintf(stderr, "Error: Failed to allocate memory for structures\n");
        return false;
    }
    
    g_structures_capacity = cap;
    fprintf(stderr, "Pre-allocated for ~%lu structures (%.2f GB)\n",
            (unsigned long)estimated_count,
            (cap * sizeof(Structure)) / (1024.0 * 1024.0 * 1024.0));
...
static bool ensure_capacity(uint64_t needed)
{
    ...
    uint64_t new_cap = g_structures_capacity;
    if (new_cap == 0)
        new_cap = 1024;
    while (new_cap < needed)
        new_cap *= 2;

    Structure *new_arr = realloc(g_structures, new_cap * sizeof(Structure));
    if (!new_arr) {
...
static uint64_t parse_file(const char *filename)
{
    ...
    while (p < end) {
        ...
        if (parse_line(line, &x, &z)) {
            ...
            g_structures[g_structures_count].x = x;
            g_structures[g_structures_count].z = z;
            g_structures_count++;
        }
    ...
    fprintf(stderr, "Parsed %lu structures (%.2f GB RAM)\n",
            (unsigned long)g_structures_count,
            (g_structures_count * sizeof(Structure)) / (1024.0 * 1024.0 * 1024.0));

    return g_structures_count;
}

/* ============================================================================
 * Sorting - compute cell coords on the fly to save memory
 * ========================================================================== */

static int compare_by_cell(const void *a, const void *b)
{
    const Structure *sa = (const Structure *)a;
    const Structure *sb = (const Structure *)b;
...
{
    /* Use larger cells to reduce count (4× radius = 16× fewer cells) */
    int64_t cell_size = radius * CELL_SIZE_MULTIPLIER;
    g_cell_size = cell_size;
    
    fprintf(stderr, "Building spatial index (cell size: %ld blocks = %d× radius)...\n",
            (long)cell_size, CELL_SIZE_MULTIPLIER);
    ...
    /* Sort by cell (computes cell coords on-the-fly) */
    fprintf(stderr, "  Sorting %lu structures by cell...\n", (unsigned long)g_structures_count);
    qsort(g_structures, g_structures_count, sizeof(Structure), compare_by_cell);
    fprintf(stderr, "  Sort complete\n");

    /* Count unique cells */
    fprintf(stderr, "  Counting unique cells...\n");
    uint64_t num_cells = 1;
    int64_t prev_cx = coord_to_cell(g_structures[0].x, cell_size);
    int64_t prev_cz = coord_to_cell(g_structures[0].z, cell_size);
    
    for (uint64_t i = 1; i < g_structures_count; i++) {
        int64_t cx = coord_to_cell(g_structures[i].x, cell_size);
        int64_t cz = coord_to_cell(g_structures[i].z, cell_size);
    ...
    fprintf(stderr, "  Found %lu unique cells (avg %.1f structures/cell)\n",
            (unsigned long)num_cells, (double)g_structures_count / num_cells);

    /* Allocate cell array */
    fprintf(stderr, "  Allocating cell index (%.2f MB)...\n",
            (num_cells * sizeof(CellEntry)) / (1024.0 * 1024.0));
    
    g_cells = malloc(num_cells * sizeof(CellEntry));
    if (!g_cells) {
        fprintf(stderr, "Failed to allocate cell array\n");
        return false;
    }
    g_cells_count = num_cells;

    /* Build cell entries */
    fprintf(stderr, "  Building cell entries...\n");
    uint64_t cell_idx = 0;
    uint64_t cell_start = 0;
    prev_cx = coord_to_cell(g_structures[0].x, cell_size);
    prev_cz = coord_to_cell(g_structures[0].z, cell_size);
    
    for (uint64_t i = 1; i <= g_structures_count; i++) {
        bool is_new_cell = (i == g_structures_count);
        if (!is_new_cell) {
            int64_t cx = coord_to_cell(g_structures[i].x, cell_size);
            int64_t cz = coord_to_cell(g_structures[i].z, cell_size);
            is_new_cell = (cx != prev_cx || cz != prev_cz);
            if (is_new_cell) {
                prev_cx = cx;
                prev_cz = cz;
            }
        ...
        if (is_new_cell) {
            g_cells[cell_idx].cellX = coord_to_cell(g_structures[cell_start].x, cell_size);
            g_cells[cell_idx].cellZ = coord_to_cell(g_structures[cell_start].z, cell_size);
    ...
    /* Build hash table */
    g_hash_table_size = 1 << 20;  /* 1M buckets */
    while (g_hash_table_size < num_cells * 2 && g_hash_table_size < (1ULL << 26))
        g_hash_table_size *= 2;
    
    fprintf(stderr, "  Building hash table (%lu buckets, %.2f MB)...\n",
    ...
    double total_mem = (g_structures_count * sizeof(Structure) + 
                       num_cells * sizeof(CellEntry) + 
                       g_hash_table_size * sizeof(uint32_t)) / (1024.0 * 1024.0 * 1024.0);
    fprintf(stderr, "  Index complete. Total memory: %.2f GB\n", total_mem);
...
static void output_group(FILE *out, pthread_mutex_t *lock,
                         Structure *structures, uint32_t *group, int count)
{
    pthread_mutex_lock(lock);

    fprintf(out, "Group of %d:\n", count);
    for (int i = 0; i < count; i++) {
        fprintf(out, "  (%d, %d)\n", structures[group[i]].x, structures[group[i]].z);
    }

    double cx = 0, cz = 0;
    for (int i = 0; i < count; i++) {
        cx += structures[group[i]].x;
        cz += structures[group[i]].z;
    }
    cx /= count;
    cz /= count;
    ...
    double max_dist = 0;
    for (int i = 0; i < count; i++) {
        double dx = structures[group[i]].x - cx;
        double dz = structures[group[i]].z - cz;
...
static bool is_valid_group(Structure *structures, uint32_t *group, int count, int64_t radius_sq)
{
    double cx = 0, cz = 0;
    for (int i = 0; i < count; i++) {
        cx += structures[group[i]].x;
...
{
    Structure *structures = work->structures;
    ...
    /* With cell_size = 4*radius, need to search ±1 cells for structures within 2*radius */
    int search_range = (CELL_SIZE_MULTIPLIER + 1) / 2 + 1;
    
    /* Collect structures from neighborhood */
    ...
    /* For each structure in this cell */
    for (uint32_t ci = 0; ci < cell->count; ci++) {
        uint32_t base_idx = cell->start + ci;
        
        /* Only consider structures in THIS cell as the minimum-index anchor */
        /* This prevents duplicate groups */
        
        /* Find candidates: neighbors with larger index within 2*radius */
        uint32_t candidates[2048];
        uint32_t num_cand = 0;
        
        for (uint32_t ni = 0; ni < num_neighbors && num_cand < 2048; ni++) {
            uint32_t idx = neighbors[ni];
            if (idx <= base_idx)
                continue;
            if (dist_sq(&structures[base_idx], &structures[idx]) <= max_pair_dist_sq) {
        ...
        if (num_cand >= 3) {
            for (uint32_t i = 0; i < num_cand - 2; i++) {
                for (uint32_t j = i + 1; j < num_cand - 1; j++) {
                    if (dist_sq(&structures[candidates[i]], &structures[candidates[j]]) > max_pair_dist_sq)
                        continue;

                    for (uint32_t k = j + 1; k < num_cand; k++) {
                        if (dist_sq(&structures[candidates[i]], &structures[candidates[k]]) > max_pair_dist_sq)
                            continue;
                        if (dist_sq(&structures[candidates[j]], &structures[candidates[k]]) > max_pair_dist_sq)
                            continue;

                        uint32_t group[4] = { base_idx, candidates[i], candidates[j], candidates[k] };
                        if (is_valid_group(structures, group, 4, radius_sq)) {
                            output_group(work->output, work->output_lock, structures, group, 4);
                            work->groups_found_4++;
                        }
                    }
                }
        ...
        for (uint32_t i = 0; i < num_cand - 1; i++) {
            for (uint32_t j = i + 1; j < num_cand; j++) {
                ...
                if (is_valid_group(structures, group, 3, radius_sq)) {
                    output_group(work->output, work->output_lock, structures, group, 3);
                    work->groups_found_3++;
                }
            }
        }
...
int main(int argc, char **argv)
{
    ...
    printf("=== Structure Group Finder (Memory-Optimized) ===\n\n");
    printf("Finds groups of 3 or 4 structures within a specified radius.\n");
    printf("Optimized for large datasets with limited RAM.\n\n");
    ...
    printf("Use multithreading? [Y/n] (detected %d cores): ", available_cores);
    fflush(stdout);
    char mt_buf[64];
    int use_mt = 1;
    ...
    printf("  Cell size: %ld blocks (%d× radius)\n", (long)(radius * CELL_SIZE_MULTIPLIER), CELL_SIZE_MULTIPLIER);
    printf("  Threads: %d\n", num_threads);
    printf("\n");
    ...
    if (count == 0) {
        fprintf(stderr, "No structures found\n");
        cleanup();
        return 1;
    }
    ...
    fprintf(output, "Total structures: %lu\n\n", (unsigned long)count);

    printf("\nSearching for groups of 3 and 4 structures...\n");
    ...
    const uint32_t BUF_SIZE = 131072;
    ...
    for (int i = 0; i < num_threads; i++) {
        ...
        work[i].cell_size = radius * CELL_SIZE_MULTIPLIER;
